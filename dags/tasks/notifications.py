...
from plugins.utils.storage import upload
from plugins.utils.config import DATA_BUCKET, ARCHIVE_FOLDER as S3_ARCHIVE_FOLDER
...
def push_logs_to_s3(log_file_path: str) -> dict:
    ...
    upload(log_file_path, key, bucket=DATA_BUCKET)
    s3_path = f"s3://{DATA_BUCKET}/{key}"
...
def archive_data(file_path: str) -> dict:
    ...
    upload(file_path, key, bucket=DATA_BUCKET)
    s3_path = f"s3://{DATA_BUCKET}/{key}"
