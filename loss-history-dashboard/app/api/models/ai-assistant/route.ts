...
export async function POST(request: NextRequest) {
  try {
    ...
    if (message.toLowerCase().includes('best model')) {
      response = 'Based on the comparison metrics, the XGBoost Fast Decay (v1.5) is currently the best performing model with an F1 score of 0.895 and AUC of 0.95. It shows a 7.8% improvement over the baseline for F1 score and 4.4% for AUC.';
    } 
    else if (message.toLowerCase().includes('improvement') || message.toLowerCase().includes('better')) {
      response = 'The XGBoost Fast Decay model shows the most significant improvement over the baseline XGBoost model, with metrics improved by 7-8.5% across accuracy, precision, recall, and F1 score. The XGBoost Equal Weight model also performs well with a 4-5% improvement.';
    } 
      ...
      response = 'Based on the comparison data, I recommend using the XGBoost Fast Decay model for production as it shows the best overall performance. However, if interpretability is important, you might consider the XGBoost Equal Weight model, which still shows good performance while being less complex.';
    }
    else if (message.toLowerCase().includes('feature') || message.toLowerCase().includes('important')) {
      response = 'The "claim_amount" and "property_age" are the most influential features across all models. The XGBoost Fast Decay appears to rely more heavily on "location_risk_score" than other models do. This might explain its improved performance, as it\'s better utilizing the geographic risk indicators.';
    }
    else {
      response = 'I\'ve analyzed the model comparison data. The XGBoost Fast Decay model is outperforming others with higher precision and recall. Would you like specific metrics, feature importance details, or recommendations for improving model performance?';
    }
